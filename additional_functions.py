...
def cat_to_int (df: pd.DataFrame, columns: list) -> pd.DataFrame:
    ...
    for column in columns:
        df[column] = df[column].str.extract(
...
def replace_int (df: pd.DataFrame, column: str, value_1: int, value_2: int, value_3: int):

    df.loc[(df[column] < value_1) | (
                df[column] > value_2), column] = value_3

    return df


def replace_emp_length(df: pd.DataFrame, column: str) -> pd.DataFrame:
    df = df.replace({column: {'4 years': 4, '< 1 year': 0, '1 year': 1, '3 years': 3, '2 years': 2,
                              '10+ years': 10, '9 years': 9, '5 years': 5,
                              '7 years': 7, '6 years': 6, '8 years': 8}})
...
def group_for_hypothesis (df: pd.DataFrame, column_1: str, column_2: str, column_3: str) -> pd.DataFrame:
...
def median_two_columns(df: pd.DataFrame, column_1: str, column_2: str, column_3: str) -> pd.DataFrame:
...
def employment_acc(df: pd.DataFrame, columns: str) -> pd.DataFrame:
    ...
    engineering = ['engineer', 'technician', 'mechanic', "technologist", "architect", "mechanical"]

    artist = ['artist', "art", "writer", "editor", "painter", "producer",
              "media", "design", "designer", "culture", "entertainment", "stylist", "teller"]

    business = ['sales', "director", "manager", "director", "associate", "business",
                "account", "consultant", "president", "management", "owner",
                "administrator", "administrative", "executive", "financial", "banker",
                "vp", "receptionist", "ceo", "dealer", "manger", "administration",
                "assistant", "finance", "cfo", "hr", "trainer", "bank", "planner",
                "recruiter", "training", "agent", "instructor", "secretary", "buyer", "human", "realtor", "broker"]

    education = ["school", "university", "education", "physician", "coach", "schools",
                 "dean", "professor", "teacher", "principal","bookkeeper", "educator",
                 "librarian", "caregiver", "examiner"]

    government = ["officer", "clerk", "police", "inspector", "counselor", "sergeant",
                  "investigator", "firefighter", "sheriff", "fire", "federal", "detective", "deputy", "captain", "army"]

    health = ["nurse", "therapist", "medical", "health", "clinical", "hospital",
              "dental", "patient", "hygienist", "pharmacist", "pharmacy",
              "paramedic", "nursing", "healthcare", "pathologist", "care"]

    tech = ["analyst", "tech", "senior", "lead", "support", "it", "developer",
            "server", "admin", "software", "programmer", "data", "processor",
            "system", "computer", "scientist", "technology", "network", "web", "tech.", "chemist"]

    worker = ['specialist', 'truck', 'maintenance', 'foreman', "electrician",
              "services", "production", "machine", "controller", "carrier", "warehouse",
              "machinist", "bus", "cashier", "attendant", "dispatcher", "carpenter", "welder",
              "flight", "delivery", "shipping", "forklift", "journeyman", "transportation", "assembler",
              "pilot", "assembly", "conductor", "crew", "courier", "electrical", "worker", "laborer",
              "handler", "logistics", "construction", "labor", "plumber", "electric", "air", "loader"]

    law = ["representative", "attorney", "paralegal", "superintendent", "advisor", "insurance",
           "county", "legal", "adjuster", "billing", "law"]

    all_words = law + worker + tech + health + government + education + business + artist + engineering
    ...
    df.loc[df[columns].str.contains('|'.join(artist)), columns] = 'Arts, culture and entertainment'
    df.loc[df[columns].str.contains('|'.join(business)), columns] = 'Business, management and administration'
    ...
    df.loc[df[columns].str.contains('|'.join(worker)), columns] = 'Maintenance, workers and transport'
...
def loan_reason(df: pd.DataFrame, columns: str) -> pd.DataFrame:
    ...
    small_business = ['refinancing', 'financing', 'business', "small_business",
                      "refinance", "starting", "project", "startup", "start-up", "company",
                      "shop", "bussiness", "bussines", "production", "entrepreneur"]

    debt_consolidation = ['debt!', "consolodation", "dept", "debts", "consolidating", "payoff", "consolidate",
                          "debt_consolidation", "consolidation", "debt", "consolidated",
                          "consilidation", "consolodate", "loan"]

    major_purchase = ['buying', "purchase", "major_purchase", "expenses",
                      "personal", "motorcycle", "life", "club", "buy", "dream", "christmas",
                      "restaurant", "bike", "purchasing", "boat", "computer", "yamaha", "engine"]

    credit_card = ["creditcard", "debit", "credit_card", "card", "credit"]

    home_improvement = ["home_improvement", "repairs", "improvements", "kitchen", "roof",
                        "improve", "room", "renovation", "bathroom", "swimming", "basement", "fixing",
                        "furniture", "building", "pool", "repair", "fix"]

    house = ["land", "housing", "rental", "apartment", "estate", "property", "home", "rent", "studio"]
    vacation = ["trip", "travel", "trailer", "vacation"]

    medical = ['medical', 'dental', 'surgery', 'nursing', "care", "rehab", "health", "hospital", "cancer"]

    car = ["car", "truck", "track", "vehicle", "honda", "auto", "automobile", "bmw", "toyota", "ford"]

    educational = ["teacher", "study", "learning", "university", "college", "education",
                   "educational", "student", "school", "tuition", "degree", "course", "graduation", "academy"]

    moving = ["moving", "move", "relocation", "transportation"]

    renewable_energy = ["energy", "solar", "green", "renewable_energy", "advisor", "insurance",
                        "county", "legal", "adjuster", "billing", "law"]

    wedding = ["married", "wife", "ring", "engagement", "divorce", "wedding", "county", "legal",
               "adjuster", "billing", "law", "marriage", "honeymoon"]

    other = wedding + renewable_energy + moving + educational + car + vacation + vacation + \
            house + home_improvement + credit_card + major_purchase + debt_consolidation + small_business

    df.loc[~df[columns].str.contains('|'.join(other)), columns] = 'other'
    df.loc[df[columns].str.contains('|'.join(wedding)), columns] = 'wedding'
    df.loc[df[columns].str.contains('|'.join(renewable_energy)), columns] = 'renewable_energy'
    ...
    df.loc[df[columns].str.contains('|'.join(small_business)), columns] = 'small_business'
    df.loc[df[columns].str.contains('|'.join(debt_consolidation)), columns] = 'debt_consolidation'
    df.loc[df[columns].str.contains('|'.join(major_purchase)), columns] = 'major_purchase'
    df.loc[df[columns].str.contains('|'.join(credit_card)), columns] = 'credit_card'
    df.loc[df[columns].str.contains('|'.join(home_improvement)), columns] = 'home_improvement'
...
def date_extract(df: pd.DataFrame, column_old: str, column_new1: str, column_new2: str) -> pd.DataFrame:
    ...
    df[column_new2] = df[column_new2].str.lower().map(lambda m: lower_ma.index(m)).astype('int')

    return df

def rejected_date_separate(df, old_column: str, new_column1: str, new_column2: str) -> pd.DataFrame:
...
def rename_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.rename(columns = {'Amount Requested':'loan_amnt', 'Loan Title':'purpose', 'Risk_Score':'fico_range',
                         'Debt-To-Income Ratio':'dti',   'State':'addr_state',
                         'Employment Length':'emp_length'}, inplace = True)
...
def base_line(X: pd.DataFrame, y: pd.DataFrame, numeric_scaler: np.array, preprocessor: np.array) -> pd.DataFrame:
    """
    ...
    classifiers = ["XGB classifier", "Random Forest", "Logistic regression", "LGBM classifier"]
    models = [
        XGBClassifier(),
...
def base_line_multi(X: pd.DataFrame, y: pd.DataFrame, preprocessor: np.array) -> pd.DataFrame:
    """
    Takes x and y dataframes.
    Perform cross validation with different models.
    Returns table with metrics and results
...
def test_data(categorical_data, numerical_data, outputs) -> list:
    ...
    return categorical_train_data, categorical_test_data, numerical_train_data,\
           numerical_test_data, train_outputs, test_outputs
...
class Plots:
  ...
  def map_plot (self):
      ...
      fig = px.scatter_geo(
          locations=self.first_df['addr_state'], locationmode="USA-states", scope="usa",
      ...
      fig_b = px.scatter_geo(
          locations=self.second_df['State'], locationmode="USA-states", scope="usa", size=self.second_df['total'],
  ...
  def kde_plot(self, column_1: str, column_2: str, x_name: str, first_label: str, second_label: str) -> None:
      """
  ...
  def strip_plot_sub(self, column_1: str, column_2: str, column_3: str, column_4: str, title_1: str, title_2: str) -> None:
  ...
  def line_plot(self, column_1: str, column_2: str, column_3: str) -> None:
      ...
      sns.lineplot(data=self.first_df, x=column_1, y=column_2, color='#2e8b57', ax=ax[0], )

      sns.lineplot(data=self.second_df, x=column_3, y=column_2, color='#5651c5', ax=ax[1])

      ax[0].set(xlabel=" ", ylabel=column_2)
      ax[1].set(xlabel=column_3, ylabel=column_2)
      fig.legend(["Accepted loans", "Rejected loans"], loc='upper left', bbox_to_anchor=(0.1, 0.85))
      sns.despine()
      plt.show()

  def strip_plot(self, column_1: str, column_2: str, xlable: str, ylabel: str) -> None:
      sns.set_style("ticks")
      plt.figure(figsize=(16, 7))
  ...
  def box_plot(self, column_1: str, column_2: str) -> None:
      sns.set_style("ticks")
      plt.figure(figsize=(16, 7))
      sns.boxplot(data=self.first_df, x=column_1, y=column_2, palette='PRGn', showmeans=True,
                  meanprops={"marker": "o",
  ...
  def bar_with_hue(self, column_1: str, column_2: str, column_3: str, title: str) -> None:
      plt.figure(figsize=(16, 7))
      sns.set_style("ticks")

      sns.barplot(data=self.first_df, x=column_1, y=column_2, hue=column_3, palette='PRGn')
      plt.yscale('log')
  ...
  def dist_plot_hue(self, column_1: str, column_2: str, x_label: str) -> None:
     ...
     palette = ["#7800FF", "#D5B1FE", "#9F60DE", "#000000", "#0A6C00",
                "#72B36C", "#ADFFA5"]
     sns.set_style("ticks")
  ...
  def bar_plot(self, column_1: str, column_2: str, title: str) -> None:
      ...
      splot = sns.barplot(data=self.first_df, x=column_1, y=column_2, palette='PRGn', ci=None)
...
def conf_matrix(model, X, y, title1, title2) -> None:
    ...
    ax[1].plot(lr_recall, lr_precision, color="#7800FF", lw=2, label="PR curve (AP  %.2f)" % ap)
    ax[1].fill_between(
        lr_recall, lr_precision, y2=np.min(lr_precision), color="#7800FF", alpha=0.3, hatch="/"
    )
...
def neural_conf_matrix(test_outputs, y_val, aggregated_losses, epochs, title1, title2) -> None:
  fig, ax = plt.subplots(1, 2, figsize=(16, 7))
  plt.rc("font", size=9)
  sns.heatmap(confusion_matrix(test_outputs, y_val, normalize='true'),
              annot=True,
              fmt=".2%",
              ax=ax[0],
              cmap="PRGn",
